<template>
    <b-container fluid>
        <crud-cabecera titulo="Tipos de Encuestas" titulo_singular="Tipo de encuesta" ruta="encuestas_tipos_nuevo" ver_boton_nuevo=true></crud-cabecera>
        <b-row>
        ...
        <b-table responsive striped hover :items="datos" :fields="fields" :filter="filter" :current-page="currentPage" :per-page="perPage" @filtered="onFiltered">
            <template slot="actions" slot-scope="row">
                <b-link :to="{name: 'encuestas_tipos_vista', params: {id : row.item.id }}"><i class="fa fa-eye"></i></b-link>
                <b-link :to="{name: 'encuestas_tipos_editar', params: {id : row.item.id }}"><i class="fa fa-edit"></i></b-link>
            </template>
        </b-table>
...
<script>
    ...
    export default {
        ...
        methods: {
            getDatos: function() {
                this.loading = true
                axios.get('/auth/encuestas/tipos')
                    .then((response) => {
